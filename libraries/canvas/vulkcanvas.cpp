...
void VulkCanvas::line(Vec3d p1, Vec3d p2, mssm::Color c)
{
...
void VulkCanvas::polygon(const std::vector<Vec3d> &points, mssm::Color border, mssm::Color fill)
{
    if (points.size() != 3) {
...
{
    setModelMatrixRotate(center, angle);
    image(center, img, src, srcw, srch);
    resetModelMatrix();
}
...
{
    setModelMatrixRotate(center, angle);
    image(center, w, h, img, src, srcw, srch);
    resetModelMatrix();
}
