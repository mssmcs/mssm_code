...
namespace SVG {
    ...
    namespace util {
        ...
        inline std::vector<Point> convex_hull(std::vector<Point>& points) {
            ...
            for (size_t i {0}; i < points.size(); i++)
                if (points[i].x < points[left].x) left = (int)i;
    ...
    protected:
        ...
        virtual std::string svg_to_string(
            const size_t indent_level) const; /** SVG string corresponding to this element */
        virtual std::string tag() const = 0; /** The SVG tag of this element */
    ...
    public:
        ...
        protected:
            ...
            std::string tag() const override { return "style"; };
        };
    ...
    protected:
        std::string tag() const override { return "svg"; }
    };
    ...
    public:
        ...
        inline void start(Vec2d coord) {
            this->start(coord.x, coord.y);
        }
        ...
        inline void line_to(T x, T y) {
            ...
            else
            {
                ...
                this->points.push_back(Vec2d(x, y));
            }
        }

        inline void line_to(Vec2d coord) {
            this->line_to(coord.x, coord.y);
        }
        ...
        inline void curve_to(double rx, double ry, double r, int bf, int af, Vec2d coord) {
            this->curve_to(rx, ry, r, bf, af, coord.x, coord.y);
        }
    ...
    protected:
        ...
        std::string tag() const override { return "path"; }
    ...
    public:
        ...
        Text(Vec2d xy, std::string _content) :
                Text(xy.x, xy.y, _content) {};
    ...
    public:
        using Element::Element;
    protected:
        std::string tag() const override { return "g"; }
    };
    ...
    public:
        ...
        Line(Point x, Point y) : Line(x.x, y.x, x.y, y.y) {};
        ...
        Vec2d along(double percent);

    protected:
        Element::BoundingBox get_bbox() override;   
        std::string tag() const override { return "line"; }
    };
    ...
    public:
        ...
        Element::BoundingBox get_bbox() override;
    protected:
        std::string tag() const override { return "rect"; }
    };
    ...
    public:
        ...
        Circle(Vec2d xy, double radius) : Circle(xy.x, xy.y, radius) {};
    ...
    protected:
        std::string tag() const override { return "circle"; }
    };
    ...
    public:
        ...
        Ellipse(double cx, double cy, double radiusX, double radiusY) :
                Shape({
                        { "cx", to_string(cx) },
                        { "cy", to_string(cy) },
        { "rx", to_string(radiusX) },
        { "ry", to_string(radiusY) }
                }) {
        };

        Ellipse(Vec2d xy, double radiusX, double radiusY) : Ellipse(xy.x, xy.y, radiusX, radiusY) {};
        double radiusX() { return this->find_numeric("rx"); }
        double radiusY() { return this->find_numeric("ry"); }
        virtual double x() override { return this->find_numeric("cx"); }
        virtual double y() override { return this->find_numeric("cy"); }
        virtual double width() override { return this->radiusX() * 2; }
        virtual double height() override { return this->radiusY() * 2; }
        Element::BoundingBox get_bbox() override;

    protected:
        std::string tag() const override { return "ellipse"; }
    };
    ...
    protected:
        std::string tag() const override { return "polygon"; }
    };
...
inline Element::BoundingBox Path::get_bbox()
{
    ...
    for(auto p : points)
    {
        if(p.x < tmp.x1) tmp.x1 = p.x;
        if(p.y < tmp.y1) tmp.y1 = p.y;
        if(p.x > tmp.x2) tmp.x2 = p.x;
        if(p.y > tmp.y2) tmp.y2 = p.y;
    }
...
inline Element::BoundingBox Ellipse::get_bbox() {
    double x = this->x(), y = this->y(), radiusx = this->radiusX(), radiusy = this->radiusY();
    ...
    inline Vec2d Line::along(double percent) {
